...
public class BstMultiset<T> extends Multiset<T>
{
	...
	public void deleteRoot(){
		Node root = mRoot; 
		Node newRoot = mRoot.mRightChild;
		Node tempNode; 
		
		if(root.mRightChild != null) {
			tempNode = findSmallest(root.mRightChild);
			tempNode.mLeftChild = root.mLeftChild;
			mRoot = newRoot;
			root = null;
		}
		else mRoot = root.mLeftChild;
	}
	...
	public void removeRight(Node root){
		...
		else if(root.mRightChild.count == 1){			
	...
	public void removeLeft(Node root){
		...
		if(root.mLeftChild.count > 1){
			root.mLeftChild.count--;
		}
	
		else if(root.mLeftChild.count == 1){
			deleteNodeLeft(root);
		}				
	}
	...
	public void removeOne(T item) {
		// Implement me!
		if(mRoot != null){
			if(((String)item).compareTo(((String)mRoot.mKey)) == 0){
				removeRoot();
			}
	...
	public void print(PrintStream out, Node root) {
		if (root == null) {
			return;
		}
		
		print(out, root.mLeftChild);
		// In general, might be better to use a function object here, but for this lab, we just print to stdout
		System.out.println(root.mKey + printDelim + root.count);
		print(out, root.mRightChild);
	} // end of print()
